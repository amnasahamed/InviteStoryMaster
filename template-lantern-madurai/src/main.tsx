...
function App(){
 ...
 return <main ref={page}>
  ...
  {!entered&&<div className={'welcome '+(introPlaying?'intro-playing':'')}><video ref={introVideo} className="welcome-video" muted playsInline preload="auto" poster="https://media.invitestory.in/lantern-madurai/assets/entrance-first-frame.png" onEnded={()=>setEntered(true)} aria-label="Aarav and Ananya's illustrated wedding invitation opening"><source src="https://media.invitestory.in/lantern-madurai/assets/aarav-ananya-opening.mp4" type="video/mp4"/></video><div className="welcome-shade"/><div className="garland-edge"/>{!introPlaying&&<Bloom onDone={()=>{const v=introVideo.current;if(!v)return;v.currentTime=0;setIntroPlaying(true);void v.play().catch(()=>setIntroPlaying(false))}}/>}{introPlaying&&<button className="skip-intro" onClick={()=>setEntered(true)}>Skip intro</button>}</div>}
  <section className="hero">
   <motion.img className="hero-art" src="https://media.invitestory.in/lantern-madurai/assets/invitation-reference.png" alt="Aarav and Ananya before a South Indian temple, framed by jasmine and lotus flowers" style={{y:reduce?0:templeY}} />
  ...
  <section className="ceremony-moment"><div className="floating-glass lotus-one">✿</div><div className="floating-glass lotus-two">❀</div><div className="floating-diya">◒</div><div className="ceremony-image"><img src="https://media.invitestory.in/lantern-madurai/assets/haldi-mehendi.png" loading="lazy" alt="Aarav and Ananya sharing a joyful haldi and mehendi moment"/></div><Reveal className="ceremony-caption"><p className="script">Painted in sunshine</p><h2>Before forever begins</h2><p>Turmeric on our cheeks, henna on our hands, and every favorite person gathered close.</p><span>Swipe through our celebrations below</span></Reveal></section>
