  ...
  return (
    <div className="paper-bg text-ink relative min-h-screen overflow-x-hidden">
      ...
      <main className="relative z-10">
        <Hero />
        {invitation.sections?.countdown !== false && <Countdown />}
        <InviteText />
        {invitation.sections?.story !== false && <Story />}
        {invitation.sections?.events !== false && <Events />}
        {invitation.sections?.venue !== false && <Venue />}
        {invitation.sections?.gallery !== false && <Gallery />}
      </main>
      <FooterBlessing />
    </div>
  );
