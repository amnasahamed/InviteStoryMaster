import { Church, Wine, Clock, Shirt } from "lucide-react"
import config from "@/config"
import SectionHeading from "@/components/SectionHeading"
import Reveal from "@/components/Reveal"

/**
 * SECTION 4 · Wedding Details — ceremony, reception, dress code.
 */
export default function Details() {
  const d = config.details
  const cards = [
    { icon: Church, title: d.ceremony.title, venue: d.ceremony.venue, time: d.ceremony.time, note: d.ceremony.note },
    { icon: Wine, title: d.reception.title, venue: d.reception.venue, time: d.reception.time, note: d.reception.note },
  ]
      ...
      <div className="grid gap-6 sm:grid-cols-2 sm:gap-8">
        {cards.map((c, i) => (
          <Reveal key={c.title} delay={i * 0.12}>
            <div className="group photo-frame h-full rounded-2xl bg-ivory/80 p-7 text-center transition-transform duration-500 hover:-translate-y-1.5">
              ...
              <p className="mt-1 font-body text-sm font-medium text-ink/80">{c.venue}</p>
              <div className="mx-auto my-3 flex items-center justify-center gap-2 text-gold">
                <Clock className="h-3.5 w-3.5" strokeWidth={1.5} />
                <span className="font-body text-xs uppercase tracking-[0.18em]">{c.time}</span>
              </div>
              <p className="font-body text-xs font-light italic text-muted-foreground">{c.note}</p>
            </div>
          </Reveal>
        ))}
      </div>
